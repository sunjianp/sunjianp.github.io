...
/*
 * el 元素
 * target 添加样式及目标点类型 {styleName： value}
 * time 动画持续时间 ms
 * type 动画类型 string
 * callback 回调函数
 * 
 ...
 * 移动端滑屏组件
  init{
  	...
  	dir: 'x'||'y'||'both' 滚动方向,
  	isOver: true/false 是否添加超出范围限制，true：不能超出范围；false：不加限制可以一直地进行滑动,
  	offMove: true/false true关闭元素移动 但是iScroll还在一直进行变化,
